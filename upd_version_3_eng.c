...
void main(void)
{
    WDTCTL = WDTPW + WDTHOLD;   // we don't need auto reset
    P1OUT = 0; // ouput of all P1 pins is logical 0
    P1DIR = BIT0^BIT6^BIT2; // enable P1.0, P1.6, P1.2 pins
/*
** I will use this array for storing our random numbers, cause i don't want to reach double and
** more in a row significant stimulations
** Let's init them
*/
    check[0] = 10;
    check[1] = 0;
...
__interrupt void COMPA_ISR(void)
{
  ...
  {
      if (check[1] == 10 && check[0] == 10)
          check[0] = 8;
      else
      {
          seed = ((seed * a + c) / 65536) % m; // take our "random" number
          check[0] = seed % 9; // making a num for stimulation (depends on our BCI)
      }
      if (check[0] == 8 && check[0] == check[1])
          check[0] = 10;
      if (check[0] == 8) // target stimulation
      {
