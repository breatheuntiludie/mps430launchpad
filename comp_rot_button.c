...
/*
**  turn on P1.3 as button for changing time_rot
*/
    /*P1REN |= BIT3; // enable pull up/down resistor for P1.3
    P1OUT |= BIT3; // enable P1.3
    P1IE = BIT3; // make P1.3 as an interrupt pin
    P1IFG &= ~BIT3;
    P1IES = BIT3;*/
/*
...
*/
    ...
    _BIS_SR(LPM0_bits + GIE); // turn on saving mode + allow interruptions
}
...
__interrupt void Port_1(void)
{
    ...
    if (key == 0 && (P1IFG & BIT3) != 0)
    {
        ...
        key++;
        P1IFG &= ~BIT3; // P1IFG -> Port1 Interrupt Flag (disabled now)
    }
    else if (key == 1)
    {
        P1OUT ^= LED1 ^ LED2; // light both LEDs (1)
